        ...
        try:
            match http_type:
                case 'http':
                    ...
                    proxy_available = [f"http://{obj.proxy()}" for obj in query_proxy_available]
                ...
                case 'https':
                    ...
                    redis_store.delete(http_type)
                    proxy_available = [f"https://{obj.proxy()}" for obj in query_proxy_available]
                    redis_store.sadd(http_type, *proxy_available)
                    return
                case _:
                    return
        ...
        try:
            def judge_http_proxy(proxy):
                ...
                try:
                    ...
                    doc = pq(resp.content.decode('utf-8'))
                    if doc('title').text() == '百度一下，你就知道':
